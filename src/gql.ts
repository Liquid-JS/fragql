...
function setNodeWithuniqueKey(
    node: ExecutableNode,
    map: Map<string, ExecutableNode>,
    prefix = '',
    unique = false
): string {
    let key = prefix.toLowerCase().replace(/[^a-z]/g, '') + '_' + paramCase(node.name)
    if (map.has(key) && map.get(key) !== node) {
        if (unique) throw new TypeError(`Found multiple definitions for ${node.name}.`)
        let i = 1
        while (map.has(`${key}-${i}`) && map.get(`${key}-${i}`) !== node) i++
        key = `${key}-${i}`
    }
    map.set(key, node)
    return key
...
export function loadSchema(source: string | GraphQLSchema, reload = false) {
    ...
    else if (source instanceof GraphQLSchema) schema = source
...
export function generateNodeMetadata(node: ExecutableNode) {
    ...
    node.dependencies.forEach((dep) => (dependencyKeyMap[dep.name] = dep.key))
...
export class ExecutableNode {
    ...
    constructor(
        ...
        if (this.definition.kind == 'FragmentDefinition') this.key = setNodeWithuniqueKey(this, fragmentMap, 'fragment')
        ...
        const doc: DocumentNode = {
            kind: Kind.DOCUMENT,
            definitions: [this, ...this.dependencies].map((node) => node.definition)
        }
        ...
        if (this.definition.kind == 'FragmentDefinition') metadata.fragments[this.key].str = str
        else if (this.definition.kind == 'OperationDefinition') metadata.operations[this.key].str = str
    ...
    dispose() {
        if (this.definition.kind == 'FragmentDefinition') fragmentMap.delete(this.key)
        else if (this.definition.kind == 'OperationDefinition') operationsMap.delete(this.key)
    }
...
export class SelectionSetNode extends ExecutableNode {
    ...
    constructor(definition: ExecutableDefinitionNode, dependencies: Set<ExecutableNode>, stack?: string[]) {
...
export function addTypename(
    ...
    node.selectionSet.selections.forEach((sel) => {
        switch (sel.kind) {
            case 'Field':
                if (sel.name.value == '__typename') hasTypename = true
                if (sel.selectionSet)
                    addTypename(
                        sel as {
                            selectionSet: GqlSelectionSetNode
        ...
        if (i < captures.length) {
            if (captures[i] instanceof SelectionSetNode) {
                ...
                if (captures[i].definition.kind != 'FragmentDefinition')
                    throw new TypeError(
                        `Tempate can only capture fragment definitions, but ${captures[i].definition.kind} was found`
                    )
            ...
            captures[i].dependencies.forEach((dep) => {
